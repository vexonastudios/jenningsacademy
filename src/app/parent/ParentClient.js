...
import { getModulesForGrade, REWARD_GAMES, AUDIOBOOKS } from "@/modules/_shared/moduleTypes";
...
// ── Sortable module row ───────────────────────────────────────────────────────
function SortableModule({ module, idx, onRemove, childSchoolDays, onToggleDay, onChangeRewardGame, onChangeAudiobook }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        ...
        {/* Audiobook selector — only shown when type is Audiobook */}
        {module.type === "Audiobook" && (
          <div onPointerDown={e => e.stopPropagation()} className="mt-2">
            <select
              value={module.audiobook_id || "this-country-of-ours"}
              onChange={e => onChangeAudiobook && onChangeAudiobook(module.id, e.target.value)}
              className="text-xs font-bold bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-3 py-1.5 cursor-pointer hover:border-amber-400 transition-colors outline-none focus:ring-2 focus:ring-amber-300"
            >
              {AUDIOBOOKS.map(b => (
...
export default function ParentClient({ profiles, initialPlans = [] }) {
  ...
  const handleChangeAudiobook = async (moduleId, bookId) => {
    const updated = todaysPlan.map(mod =>
      mod.id === moduleId ? { ...mod, audiobook_id: bookId } : mod
    );
    setTodaysPlan(updated);
    if (activeChild) {
      const todayStr = new Date().toISOString().split('T')[0];
      try {
        await publishPlan({ profileId: activeChild.id, modules: updated, targetDate: todayStr });
        addToast("📚 Audiobook updated!", "save");
      } catch {
        addToast("Couldn't save — try again", "error");
      }
    }
                      ...
                      <div className="space-y-4">
                        ...
                        {todaysPlan.map((module, idx) => (
                          <SortableModule
                            ...
                            onChangeAudiobook={handleChangeAudiobook}
                          />
                        ))}
                      </div>
                    </SortableContext>
